...
export const createOrganization = asyncHandler(async (req, res) => {
  ...
  await logAudit({ userId: req.user._id, action: "org_create", details: `Created org: ${name}`, req });
...
export const updateOrganization = asyncHandler(async (req, res) => {
  ...
  if (!membership || membership.role !== "admin") {
    return res.status(403).json({ message: "Only admins can update the organization" });
  }
  ...
  await logAudit({ userId: req.user._id, action: "org_update", details: `Updated org: ${organization.name}`, req });
...
export const inviteMember = asyncHandler(async (req, res) => {
  ...
  if (!["admin", "recruiter"].includes(requestedRole)) {
    return res.status(400).json({ message: "Role must be either 'admin' or 'recruiter'" });
  }
  ...
  await logAudit({ userId: req.user._id, action: "org_member_invite", details: `Invited ${email} to org`, req });

  res.json({ message: "Member added", member: { user: userToAdd._id, email, role: role || "recruiter" } });
});
