...
import { Mic, MicOff, ArrowRight, Loader2, AlertCircle, Volume2, ArrowLeft, CheckCircle2 } from "lucide-react";
...
const CandidateInterview: React.FC = () => {
  ...
  const fetchInvite = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/interviews/candidate/accept/${token}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Invalid invite");
      setInvite(data.invite);
      setInterview(data.interview);
      setQuestions(data.interview.questions || []);
      // Store the auth token from acceptInvite response
      if (data.token) {
  ...
  const handleToggleRecording = async () => {
    if (isRecording) {
      ...
      try {
        ...
        mediaRecorder.start();
        setIsRecording(true);
      } catch { setError("Could not access microphone. Please check permissions."); }
    }
  };
  ...
  const handleEvaluate = async (audioBlob: Blob) => {
    ...
    try {
      ...
      const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:9000";
      const res = await fetch(`${apiUrl}/api/interviews/candidate/evaluate`, {
        method: "POST",
      ...
      const newResult = { question: questions[currentQuestionIndex], transcription: data.transcription || "", score: data.score, feedback: data.feedback };
  ...
  const handleNext = async () => {
    if (isRecording) { if (mediaRecorderRef.current?.state !== "inactive") mediaRecorderRef.current?.stop(); setIsRecording(false); if (streamRef.current) { streamRef.current.getTracks().forEach((t) => t.stop()); streamRef.current = null; } }
    setEvaluation(null);
    setError(null);
  ...
  const submitAllResults = async () => {
    ...
    try {
      const token = authToken || localStorage.getItem("token");
      if (!token || !invite?._id) throw new Error("Not authenticated");
      const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:9000";
      ...
      for (let i = 0; i < sessionResults.length; i++) {
        ...
        if (!result) continue;
        await fetch(`${apiUrl}/api/interviews/candidate/submit`, {
          method: "POST",
      ...
      // Still show completed even if submission fails — results were evaluated
      setCompleted(true);
    } finally {
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      <p className="text-[13px] font-medium text-muted-foreground mt-3">Loading interview...</p>
    </div>
  );
  ...
  if (error && !questions.length) return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm border border-border/80 rounded-xl bg-card p-8 text-center space-y-4">
        ...
        <h3 className="font-bold text-foreground">Could not load interview</h3>
        <p className="text-[13px] text-muted-foreground">{error}</p>
        <Button onClick={() => navigate("/")} className="w-full font-medium">Go Home</Button>
      </div>
    </div>
  );
  ...
  if (isSubmitting) return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="flex flex-col items-center gap-3">
        <Loader2 className="w-8 h-8 text-primary animate-spin" />
        <p className="text-[13px] font-medium text-muted-foreground">Submitting your responses...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm border border-border/80 rounded-xl bg-card p-8 text-center space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[13px] text-muted-foreground">The recruiter will review your responses.</p>
        </div>
        {sessionResults.length > 0 && (
          <div className="space-y-1.5">
            {sessionResults.map((r, i) => (
              <div key={i} className="flex items-center justify-between p-2.5 bg-surface rounded-lg text-[13px]">
                <span className="font-medium text-foreground">Q{i + 1}</span>
                <span className="font-bold text-primary">{r?.score || 0}/10</span>
              </div>
            ))}
          </div>
        )}
        <Button onClick={() => navigate("/")} className="w-full font-semibold">Return Home</Button>
      </div>
    </div>
  );
      ...
      <div className="border-b border-border/60 bg-surface/80 backdrop-blur-sm sticky top-14 z-30 py-3 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/")} className="p-1 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="md:col-span-5 flex flex-col">
          <div className="border border-border/80 shadow-sm bg-card rounded-xl flex-1 flex flex-col overflow-hidden">
            <div className="border-b border-border/60 px-5 py-3 bg-secondary/15">
              <span className="text-[12px] font-semibold text-foreground">{interview?.title || "Interview"}</span>
              <p className="text-[11px] text-muted-foreground mt-0.5">{interview?.targetRole}</p>
            </div>
            <div className="flex-1 p-5 flex flex-col justify-center items-center">
              ...
              <p className="text-[12px] text-muted-foreground text-center">Listen carefully, then record your response.</p>
            </div>
          ...
          <div className="border border-border/80 shadow-sm bg-card rounded-xl flex-1 flex flex-col overflow-hidden relative min-h-[340px]">
            <div className="flex-1 bg-black relative flex flex-col justify-center items-center overflow-hidden">
              {isEvaluating ? (
                <div className="absolute inset-0 bg-background/95 backdrop-blur-md z-20 flex flex-col items-center justify-center gap-3">
                  <Loader2 className="w-8 h-8 text-primary animate-spin" />
                  <p className="text-[13px] font-semibold">Evaluating your response...</p>
                </div>
              ) : evaluation ? (
                <div className="absolute inset-0 bg-background/95 backdrop-blur-md z-20 flex flex-col justify-between p-5 overflow-y-auto">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between border-b border-border pb-3">
                      <div>
                        <span className="text-[10px] font-bold text-success bg-success/10 px-2 py-0.5 rounded uppercase tracking-wider">Ready</span>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-5">
                  ...
                  <span className="text-[11px] text-white/40">{isRecording ? "Recording..." : "Ready to record"}</span>
                </div>
              )}
              ...
              {isRecording && (
                <>
                  <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-destructive/10 border border-destructive/20 px-2.5 py-1 rounded-full">
                    ...
                    <span className="text-[9px] font-bold uppercase text-white tracking-wider">Live</span>
                  </div>
            ...
            {!evaluation && (
              <div className="border-t border-border/60 p-4 bg-background flex flex-col sm:flex-row items-center justify-between gap-3">
                <p className="text-[12px] text-muted-foreground font-medium">{isRecording ? "Click stop when finished." : "Click to start recording."}</p>
                <Button variant={isRecording ? "destructive" : "default"} onClick={handleToggleRecording} disabled={isEvaluating}
