...
import { Upload, Camera, Mic, AlertCircle, FileText, CheckCircle2, ChevronRight, RefreshCw, Trash2, Volume2 } from 'lucide-react';
...
const Onboarding = () => {
    const steps: OnboardingStep[] = [
        { step: 1, title: "Resume", description: "Upload your professional background." },
        { step: 2, title: "Photo", description: "Capture a headshot for verification." },
    ...
    const startCamera = useCallback(async () => {
        try {
            ...
            if (!videoRef.current) { setCameraLoading(false); return; }
            ...
            if (err instanceof Error) {
                if (err.name === 'NotAllowedError') setError('Camera permission denied. Please allow access and try again.');
    ...
    const capturePhoto = () => {
        if (videoRef.current && videoRef.current.videoWidth > 0) {
            ...
            if (ctx) { ctx.drawImage(videoRef.current, 0, 0); setPhoto(canvas.toDataURL('image/jpeg', 0.8)); }
            stopCamera();
        } else {
    ...
    const recordAgain = () => { setAudioUrl(null); setAudioBlob(null); setIsRecording(false); };
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            let resumeBase64 = resumeFile ? await fileToBase64(resumeFile) : "";
            let audioBase64 = audioBlob ? await fileToBase64(audioBlob) : "";
            const res = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:9000"}/api/auth/onboard`, {
                method: "POST",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({ resumeName: resumeFile?.name || "", resume: resumeBase64, photo, audio: audioBase64 }),
            });
    ...
    const isNextDisabled = () => {
        switch (currentStep) {
            case 1: return !resumeFile;
            case 2: return !photo;
            case 3: return !audioUrl;
            default: return true;
        }
    };
            ...
            <div className="flex-1 flex flex-col md:flex-row max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 gap-10">
                ...
                <div className="w-full md:w-64 shrink-0 space-y-6">
                    <div className="space-y-1">
                        <h2 className="text-lg font-bold tracking-tight text-foreground">Complete your profile</h2>
                        <p className="text-[13px] text-muted-foreground leading-relaxed">
                            These calibration steps help the AI evaluate your responses accurately.
                        </p>
                    </div>
                        ...
                        {steps.map((s, idx) => {
                            ...
                            return (
                                <div key={s.step} className="flex items-start gap-3 relative">
                                    {/* Connector line */}
                                    {idx < steps.length - 1 && (
                                        <div className={`absolute left-[13px] top-[28px] bottom-0 w-px ${isDone ? "bg-primary" : "bg-border/60"}`} />
                                    )}
                                    {/* Step circle */}
                                    ...
                                    <div className="pb-6 pt-0.5">
                                        <p className={`text-[13px] font-semibold ${isActive ? "text-foreground" : isDone ? "text-muted-foreground" : "text-muted-foreground/70"}`}>
                                            {s.title}
                                        </p>
                ...
                <div className="flex-1 flex flex-col">
                    <div className="border border-border/80 rounded-xl bg-card shadow-sm flex-1 flex flex-col overflow-hidden">
                                    ...
                                    {resumeFile ? (
                                        <div className="w-full max-w-md bg-surface border border-border/60 rounded-lg p-4 flex items-center gap-4 animate-in zoom-in-95 duration-200">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[13px] font-semibold text-foreground truncate">{resumeFile.name}</p>
                                                <p className="text-[11px] text-muted-foreground">{(resumeFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                                            </div>
                                            <button onClick={() => setResumeFile(null)} className="p-1.5 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
                                                <Trash2 className="h-4 w-4" />
                                            </button>
                                        </div>
                                    ) : (
                                        <div {...getRootProps()} className="w-full max-w-md border-2 border-dashed border-border hover:border-primary/40 bg-surface/50 hover:bg-secondary/50 rounded-xl p-10 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-200 select-none group">
                                            ...
                                            <h3 className="text-sm font-semibold text-foreground mb-1">Upload your resume</h3>
                                            <p className="text-[12px] text-muted-foreground max-w-xs">PDF or Word document, up to 5 MB</p>
                                        </div>
                                    )}
                                </div>
                            )}
                                ...
                                <div className="flex flex-col items-center gap-5">
                                    {photo ? (
                                        <div className="flex flex-col items-center gap-4 animate-in zoom-in-95 duration-200">
                                            ...
                                            <Button variant="outline" size="sm" onClick={() => { setPhoto(null); startCamera(); }} className="font-medium">
                                        ...
                                        <div className="w-full max-w-md flex flex-col items-center gap-4">
                                            <div className="relative w-full aspect-video bg-black rounded-xl overflow-hidden border border-border">
                                                ...
                                                <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover ${cameraLoading ? 'opacity-0' : ''}`} />
                                            </div>
                                            <Button onClick={capturePhoto} disabled={cameraLoading} className="font-semibold px-6">
                            ...
                            {currentStep === 3 && (
                                <div className="flex flex-col items-center gap-5 w-full">
                                    <div className="w-full max-w-md bg-surface border border-border/60 rounded-lg p-4 text-center">
                                        <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-1.5">Read this aloud</p>
                                        <p className="text-[13px] font-medium text-foreground leading-relaxed italic">
                                            "I am excited to join Rehearse.io for interview preparation and verify my audio levels."
                                        </p>
                                    </div>
                                    {audioUrl ? (
                                        <div className="flex flex-col items-center gap-4 w-full max-w-md bg-card p-4 rounded-lg border border-border/80 shadow-sm animate-in zoom-in-95 duration-200">
                                            <div className="flex items-center gap-2 w-full">
                                                ...
                                                <span className="text-[13px] font-semibold">Calibration complete</span>
                                            </div>
                                            <audio src={audioUrl} controls className="w-full h-9" />
                                            <Button variant="outline" size="sm" onClick={recordAgain} className="w-full font-medium">
                                                <RefreshCw className="w-3.5 h-3.5 mr-1.5" />Re-record
                                            </Button>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            >
                                                {isRecording ? "STOP" : <><Mic className="w-5 h-5 mr-2" />RECORD</>}
                                            </Button>
                                            <p className="text-[12px] text-muted-foreground font-medium">
                                                {isRecording ? "Recording..." : "Click to record calibration"}
                                            </p>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="border-t border-border/60 px-6 py-4 flex items-center justify-between bg-secondary/10">
                            <Button variant="ghost" onClick={handleBack} disabled={currentStep === 1 || isSubmitting} className="text-muted-foreground font-semibold text-[13px]">
                                Back
                            </Button>
                            <Button
                            ...
                            >
                                {isSubmitting ? "Saving..." : (currentStep === steps.length ? "Finish" : "Continue")}
